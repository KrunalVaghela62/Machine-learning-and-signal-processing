...
function compare_error_vs_OF()
    ...
    % Storage
    mean_errors_LP = zeros(size(OF_list));
    std_errors_LP = zeros(size(OF_list));
    mean_errors_B2R2 = zeros(size(OF_list));
    std_errors_B2R2 = zeros(size(OF_list));

    % Run for each OF
    for i = 1:length(OF_list)
        OF = OF_list(i);
        ...
        for trial = 1:num_trials
            ...
            [x_rec_lp, success] = modulo_recovery_corrected(x_mod, Ts, W, Delta, E);
            if success
                x_rec_cont = sinc_interpolation(x_rec_lp, t_s, t_d, Ts);
            ...
            Ws=2*Wm*OF;
            n = -L/2 : L/2-1;
            [x_n, ~, ~, ~] = generate_BL_signal(10, L, 1/OF, n, OF);
            [~, x_dense] = sinc_interpolate_bandlimited(x_n, n, 1.05,Wm,OF);
            ...
            [~, x_dense_rec] = sinc_interpolate_bandlimited(x_rec, n, 1.05,Wm,OF);
    ...
    %% Plot with setbars (errorbars)
    figure('Name','Recovery Error vs Oversampling Factor','Units','normalized','OuterPosition',[0 0 1 1]);
    hold on;

    % LP error bars
    errorbar(OF_list, mean_errors_LP, std_errors_LP, 'b-o', ...
             'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'LP');

    % B2R2 error bars
    errorbar(OF_list, mean_errors_B2R2, std_errors_B2R2, 'r-s', ...
             'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'B2R2');

    xlabel('Oversampling Factor (OF)', 'FontSize', 20);
    ylabel('MSE (dB)', 'FontSize', 20);
    legend('LP', 'B2R2', 'Location', 'best');
    title('Comparison: LP vs B2R2 Recovery Error vs OF', 'FontSize', 24);
    grid on;
    set(gca, 'FontSize', 16);
end
...
function [x_orig_cont,t_d]=generate_original(E, W,OF)
    ...
    for m = 1 : NoS
        f = f + CoS(m) * sinc(BW / pi * t - (m - NoS/2));
    end
